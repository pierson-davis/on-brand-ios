...
Updates all file references in the Xcode project file to match the new folder structure
"""

import re
from pathlib import Path

def fix_xcode_references():
    ...
    for old_path, new_path in path_mappings.items():
        ...
        # Also update any references that might have the full era/ prefix
        old_pattern_with_prefix = f'path = era/{old_path};'
        new_pattern_with_prefix = f'path = era/{new_path};'
        
        if old_pattern_with_prefix in updated_content:
            updated_content = updated_content.replace(old_pattern_with_prefix, new_pattern_with_prefix)
            changes_made += 1
